...
# ============== 1) TIỀN XỬ LÝ ROI (NÂNG CẤP MẠNH) ==============
def preprocess_for_ocr(roi):
    """
    Nhận roi BGR -> trả về ảnh BGR đã tăng cường để dùng cho YOLO OCR.
    Các bước:
      - Gray
      - CLAHE
      - GaussianBlur
      - Adaptive Threshold (INV) + Morph close
      - Merge về 3 kênh BGR
    """
    if roi is None:
        return None
    ...
    enhanced_bgr = cv2.cvtColor(th, cv2.COLOR_GRAY2BGR)
    return enhanced_bgr
...
# ============== 2) OCR NHẬN ROI ĐÃ TĂNG CƯỜNG ==============
def ocr_plate(roi_enhanced_bgr):
    """
    Nhận trực tiếp ROI đã tăng cường (BGR) và OCR bằng YOLO.
    Trả về (text_fmt, text_raw)
    """
    res = ocr_model(roi_enhanced_bgr)
    text_raw = ""
...
def safe_markdown(ph, html):
    if html: ph.markdown(html, unsafe_allow_html=True)
    else: ph.empty()
...
with t1: ph_cam1 = st.empty()
with t2: ph_cam2 = st.empty()
with b1: ph_scene = st.empty()
with b2: ph_roi   = st.empty()
...
def render_info():
    ...
    if st.session_state.roi_in_path and os.path.exists(st.session_state.roi_in_path):
        roi_in_box.image(st.session_state.roi_in_path, caption="ROI Cam 1 (Vào) - enhanced", width=280, use_container_width=False)
    else:
        roi_in_box.empty()
    if st.session_state.roi_out_path and os.path.exists(st.session_state.roi_out_path):
        roi_out_box.image(st.session_state.roi_out_path, caption="ROI Cam 2 (Ra) - enhanced", width=280, use_container_width=False)
    else:
        roi_out_box.empty()
...
def refresh_history():
    if USE_SQL:
        try:
            ...
            # Hiển thị 'PENDING' nếu trạng thái trống
...
# ============== 3) PROCESS_STREAM: dùng ROI ENHANCED ==============
def process_stream(frame, mode="in"):
    ...
    roi_current = best[1]
    roi_enh = preprocess_for_ocr(roi_current)
    ...
    if mode == "in":
        ...
        if (not st.session_state.captured_in) and (time.time() - st.session_state.stable_start_in >= STABLE_SECONDS_IN):
            text_fmt, text_raw = ocr_plate(roi_enh if roi_enh is not None else roi_current)
            if text_fmt or text_raw:
                ...
                roi_path   = save_image(roi_enh if roi_enh is not None else roi_current, "plate_in_roi_enh")
                st.session_state.scene_path = scene_path
                st.session_state.roi_path   = roi_path
                st.session_state.roi_in_path = roi_path

                scene_img = cv2.imread(scene_path); show(ph_scene, scene_img, "3) Image_BOX")
                roi_img   = cv2.imread(roi_path);   show(ph_roi, roi_img, "4) ROI_Plates (enhanced)")
                render_info()
        ...
        if (not st.session_state.captured_out) and (time.time() - st.session_state.stable_start_out >= STABLE_SECONDS_OUT):
            ...
            if text_fmt or text_raw:
                ...
                roi_path   = save_image(roi_enh if roi_enh is not None else roi_current, "plate_out_roi_enh")
                st.session_state.scene_path = scene_path
                st.session_state.roi_path   = roi_path
                st.session_state.roi_out_path = roi_path
...
if st.session_state.cam1_on or st.session_state.cam2_on:
    ...
    if st.session_state.roi_path and os.path.exists(st.session_state.roi_path):
        roi_img = cv2.imread(st.session_state.roi_path); show(ph_roi, roi_img, "4) ROI_Plates (enhanced)")
    else:
