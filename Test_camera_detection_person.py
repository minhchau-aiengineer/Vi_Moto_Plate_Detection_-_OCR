...
# url = "rtsp://admin:CMF%402025@172.16.120.10:554/Streaming/Channels/102?transportmode=unicast"

# cap = cv2.VideoCapture(url, cv2.CAP_FFMPEG)

# # Các thuộc tính này có thể không được hỗ trợ trong mọi build; set được thì tốt
# for prop, val in [
...
# t0 = time.time()
# while (time.time() - t0) < 8 and not cap.isOpened():
#     time.sleep(0.5)

# if not cap.isOpened():
#     raise Exception("❌ Cannot open camera (check URL/credentials/network)")
...
#         break
#     cv2.imshow("RTSP", cv2.resize(frame, (640, 480)))
